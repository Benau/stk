...
void Kart::updateGraphics(float dt)
{
    ...
    {
        bool shadow_enabled = CVS->isShadowEnabled() &&
            Track::getCurrentTrack()->hasShadows();
        if (supports_shadow && shadow_enabled && m_shadow)
            m_shadow.reset();
        else if ((!supports_shadow ||!shadow_enabled) && !m_shadow)
        {
