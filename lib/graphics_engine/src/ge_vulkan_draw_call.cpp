                      ...
                      const btQuaternion& rotation, bool mirror)
{
    ...
    m_scale_x = billboard_size.X / (mirror ? -2.0f : 2.0f);
...
void GEVulkanDrawCall::addNode(irr::scene::ISceneNode* node)
{
    ...
    {
        mesh = static_cast<GEVulkanAnimatedMeshSceneNode*>(node)->getMesh();
    }
...
void GEVulkanDrawCall::generate(GEVulkanDriver* vk)
{
    ...
    std::unordered_set<std::string> sorting_keys;
    for (auto& p : m_visible_nodes)
    {
        ...
        for (auto& q : p.second)
        {
            ...
            sorting_keys.insert(sorting_key);
        }
    }

    generateDynamicSPM(vk);
    nodes_area[NULL] = 0.0f;
    for (auto& p : m_rendered_dspm)
    {
        const std::string& shader = p.first;
        const PipelineSettings& settings =
            m_graphics_pipelines[shader].m_settings;
        std::string sorting_key =
            std::string(1, settings.m_drawing_priority) + shader;
        if (sorting_keys.find(sorting_key) == sorting_keys.end())
        {
            sorting_keys.insert(sorting_key);
            visible_nodes.emplace_back(
                std::make_pair(sorting_key, Nodes()), MeshBufferMaterial());
        }
    }
...
start:
    ...
    std::unordered_map<uint32_t, int> offset_map;
    for (auto& p : visible_nodes)
    {
        VkDrawIndexedIndirectCommand cmd = {};
        std::string cur_shader = p.first.first.substr(1);
        ...
        m_cmds.push_back({ cmd, cur_shader, mb, material_id,
            use_base_vertex ? -1 : offset_map[cmd.firstInstance] });
        if (!skip_instance_key && it == cur_key.end())
             cur_key.push_back(key);
    }

    size_t object_data_padded_size = written_size;
    ...
    if (bind_mesh_textures && !m_cmds.empty())
    {
        ...
        if ((object_data_padded_size * 2) + (materials_padded_size * 2) >
            m_sbo_data->getSize())
        {
            min_size =
    ...
    else
    {
        ...
        if (object_data_padded_size * 2 > m_sbo_data->getSize())
        {
            min_size = object_data_padded_size * 2;
            goto start;
        }
    }
}   // generate
        ...
        if (getGEConfig()->m_shadow_size > 0 && sun)
        {
            ...
            case GST_SUN:
                m_shadow_fbo = new GEVulkanShadowFBO(vk,
                    getGEConfig()->m_shadow_size, this, sun);
                break;
            case GST_POINTLIGHT:
...
void GEVulkanDrawCall::createVulkanData()
{
    ...
    if (GEVulkanFeatures::supportsBindMeshTexturesAtOnce())
        sizes.back().descriptorCount = (vk->getMaxFrameInFlight() + 1) * 3 * 2;

    VkDescriptorPoolCreateInfo pool_info = {};
    pool_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    pool_info.flags = 0;
    pool_info.maxSets = ((vk->getMaxFrameInFlight() + 1) * 2) + 1;
    ...
    m_dspm_descriptor_sets.resize(set_size);
    data_layouts = std::vector<VkDescriptorSetLayout>(
        m_dspm_descriptor_sets.size(), m_data_layout);
    alloc_info.descriptorSetCount = data_layouts.size();
    alloc_info.pSetLayouts = data_layouts.data();

    if (vkAllocateDescriptorSets(vk->getDevice(), &alloc_info,
        m_dspm_descriptor_sets.data()) != VK_SUCCESS)
    {
        throw std::runtime_error("vkAllocateDescriptorSets failed for dspm "
            "layout");
    }
                                         ...
                                         VkCommandBuffer custom_cmd)
{
    ...
    if (m_sbo_data &&
        m_sbo_data->getHostMemory()[current_buffer_idx] != VK_NULL_HANDLE)
    {
    ...
    if (m_dspm_data &&
        m_dspm_data->getHostMemory()[current_buffer_idx] != VK_NULL_HANDLE)
    {
        vmaFlushAllocation(vk->getVmaAllocator(),
            m_dspm_data->getHostMemory()[current_buffer_idx], 0,
            m_dynamic_spm_padded_size);
    }

    if (m_skinning_data &&
        m_skinning_data->getHostMemory()[current_buffer_idx] != VK_NULL_HANDLE)
    {
        vmaFlushAllocation(vk->getVmaAllocator(),
            m_skinning_data->getHostMemory()[current_buffer_idx], 0,
            m_skinning_offset * sizeof(irr::core::matrix4));
    }
    m_skinning_offset = 1;
}   // uploadDynamicData
                                      ...
                                      bool& rebind_base_vertex)
{
    ...
    {
        indirect_buffer = GEVulkanDynamicBuffer::supportsHostTransfer() ?
    ...
    for (unsigned i = 0; i < m_cmds.size(); i++)
    {
        ...
        if (pipeline_change || is_last_cmd)
        {
            ...
            if (bound && m_cmds[i].m_mb != NULL)
            {
                drawCommands(cmd, cur_pipeline, current_buffer_idx,
                    indirect_buffer, indirect_offset, draw_count, pt);
            }
            indirect_offset += draw_count *
                sizeof(VkDrawIndexedIndirectCommand);

            auto it = m_rendered_dspm.find(cur_pipeline);
            if (bound && it != m_rendered_dspm.end())
            {
                int cur_mid = -1;
                for (auto& p : it->second)
                {
                    DynamicSPMData& dspm_data = p.second;
                    if (!indirect_buffer &&
                        cur_mid != dspm_data.m_material_id)
                    {
                        cur_mid = dspm_data.m_material_id;
                        bindSingleMaterial(cmd, cur_pipeline, cur_mid, pt);
                    }
                    dynamic_offsets[1] = dspm_data.m_dynamic_offset;
                    vkCmdBindDescriptorSets(cmd,
                        VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipeline_layout, 1,
                        1, &dspm_data.m_descriptor_set,
                        dynamic_offsets.size(), dynamic_offsets.data());
                    if (p.first->isDynamic())
                    {
                        rebind_base_vertex = true;
                        p.first->drawDynamicVertexIndexBuffer(cmd,
                            current_buffer_idx);
                    }
                    else
                    {
                        if (!use_base_vertex)
                            p.first->bindVertexIndexBuffer(cmd);
                        else if (rebind_base_vertex)
                        {
                            bindBaseVertex(vk, cmd);
                            rebind_base_vertex = false;
                        }
                        vkCmdDrawIndexed(cmd, p.first->getIndexCount(),
                            dspm_data.m_instance_count,
                            use_base_vertex ? p.first->getIBOOffset() : 0,
                            use_base_vertex ? p.first->getVBOOffset() : 0,
                            0);
                    }
                }
            }
...
// ----------------------------------------------------------------------------
void GEVulkanDrawCall::drawCommands(VkCommandBuffer cmd,
                                    ...
                                    GEVulkanPipelineType pt)
{
    ...
    if (indirect_buffer)
    {
        dynamic_offsets[4] = m_materials_data[pipeline].first;
    ...
    else
    {
        ...
        bindDataDescriptor(cmd, current_buffer_idx,
            dynamic_offsets);
        auto start = m_cmds.begin() + indirect_offset / indirect_size;
        auto end = start + draw_count;
        while (start != end)
        {
            DrawCallData& cur_cmd = *start;
            start++;
            if (cur_cmd.m_mb == NULL)
                continue;
            int mid = cur_cmd.m_material_id;
            if (cur_mid != mid)
            {
                cur_mid = mid;
                bindSingleMaterial(cmd, pipeline, cur_mid, pt);
            }
            bool use_base_vertex = cur_cmd.m_dynamic_offset == -1;
            if (!use_base_vertex)
            {
                dynamic_offsets[1] = cur_cmd.m_dynamic_offset;
                bindDataDescriptor(cmd, current_buffer_idx,
                    dynamic_offsets);
                cur_cmd.m_mb->bindVertexIndexBuffer(cmd);
            }
            vkCmdDrawIndexed(cmd, cur_cmd.m_cmd.indexCount,
                cur_cmd.m_cmd.instanceCount, cur_cmd.m_cmd.firstIndex,
                cur_cmd.m_cmd.vertexOffset,
                use_base_vertex ? cur_cmd.m_cmd.firstInstance : 0);
        }
    }
}   // drawCommands
                                                ...
                                                GEVulkanCameraSceneNode* cam)
{
    ...
    if (m_skinning_data_observer.expired())
    {
        if (getMasterDrawCall()->m_skinning_data)
        {
            m_update_data_descriptor_sets = true;
            m_skinning_data_observer =
                getMasterDrawCall()->m_skinning_data->getBufferObserver();
        }
    }
    ...
    for (unsigned i = 0; i < m_data_descriptor_sets.size(); i++)
    {
        ...
        VkBuffer skinning_buffer = getMasterDrawCall()->m_skinning_data ?
            getMasterDrawCall()->m_skinning_data->getHostBuffer()[i] :
            VK_NULL_HANDLE;
        if (skinning_buffer != VK_NULL_HANDLE)
        {
            sbo_info_skinning.buffer = skinning_buffer;
            sbo_info_skinning.offset = 0;
            sbo_info_skinning.range = VK_WHOLE_SIZE;
            data_set.push_back({});
            VkWriteDescriptorSet& ds = data_set.back();
            ds.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            ds.dstSet = m_data_descriptor_sets[i];
        ...
        if (bind_mesh_textures && sbo_buffer != VK_NULL_HANDLE)
        {
            ...
            sbo_info_material.offset = m_object_data_padded_size;
...
// ----------------------------------------------------------------------------
size_t GEVulkanDrawCall::getDynamicSPMSize() const
{
    size_t size = 0;
    size_t sbo_alignment = m_limits.minStorageBufferOffsetAlignment;
    for (auto& p : m_dynamic_spm_buffers)
    {
        ...
        for (auto& q : p.second)
        {
            ...
            for (irr::scene::ISceneNode* node : q.second)
            {
                ...
                else if (node->getType() == irr::scene::ESNT_BILLBOARD)
                {
                    if (m_shadow_fbo && material->m_backface_culling)
                        count += 2;
                    else
                        count += 1;
                }
...
// ----------------------------------------------------------------------------
void GEVulkanDrawCall::generateDynamicSPM(GEVulkanDriver* vk)
{
    unsigned dspm_size = getDynamicSPMSize();
    m_dynamic_spm_padded_size = 0;
    if (dspm_size == 0)
        return;

    m_update_data_descriptor_sets = m_dspm_data->resizeIfNeeded(dspm_size) ||
        m_update_data_descriptor_sets;

    int current_buffer_idx = vk->getCurrentBufferIdx();
    uint8_t* mapped_addr = (uint8_t*)m_dspm_data->getMappedAddr()
        [current_buffer_idx];
    size_t sbo_alignment = m_limits.minStorageBufferOffsetAlignment;
    size_t written_size = 0;
        ...
        for (auto& q : p.second)
        {
            ...
            for (irr::scene::ISceneNode* node : q.second)
            {
                    ...
                    node->getType() == irr::scene::ESNT_PARTICLE_SYSTEM)
                {
                    ...
                    {
                        irr::scene::IBillboardSceneNode* bb = static_cast<
                            irr::scene::IBillboardSceneNode*>(node);
                        ObjectData* obj = (ObjectData*)mapped_addr;
                        obj->init(bb, material_id, m_billboard_rotation,
                            false);
                        written_size += sizeof(ObjectData);
                        mapped_addr += sizeof(ObjectData);
                        if (m_shadow_fbo && material->m_backface_culling)
                        {
                            obj = (ObjectData*)mapped_addr;
                            obj->init(bb, material_id, m_billboard_rotation,
                                true);
                            written_size += sizeof(ObjectData);
                            mapped_addr += sizeof(ObjectData);
            ...
            unsigned instance_count = (written_size - dynamic_spm_offset) /
                sizeof(ObjectData);
            size_t padding = getPadding(written_size, sbo_alignment);
            written_size += padding;
            mapped_addr += padding;
            m_rendered_dspm[shader][buf] = { material_id,
...
// ----------------------------------------------------------------------------
irr::core::matrix4* GEVulkanDrawCall::getSkinningOffset(unsigned bone_count,
                                                        unsigned* offset)
{
    const size_t min_size = (m_skinning_offset + bone_count) *
        sizeof(irr::core::matrix4);
    if (!m_skinning_data)
    {
        m_skinning_data = new GEVulkanDynamicBuffer(
            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, min_size,
            GEVulkanDriver::getMaxFrameInFlight() + 1, 0);
    }
    m_skinning_data->resizeKeepData(min_size);

    GEVulkanDriver* vk = static_cast<GEVulkanDriver*>(getDriver());
    irr::core::matrix4* mapped_addr = (irr::core::matrix4*)
        m_skinning_data->getMappedAddr()[vk->getCurrentBufferIdx()];

    if (m_skinning_offset == 1)
    {
        static irr::core::matrix4 identity = irr::core::matrix4();
        memcpy((void*)mapped_addr, identity.pointer(),
            sizeof(irr::core::matrix4));
        mapped_addr += 1;
    }
    else
        mapped_addr += m_skinning_offset;

    *offset = m_skinning_offset;
